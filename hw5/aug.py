...
def aug_feature_dropout(input_feat, drop_percent = 0.2):
    ...
    aug = copy.deepcopy(input_feat.squeeze(0)) #copy input_feat
    drop = random.sample([i for i in range(aug.shape[1])], int(aug.shape[1] * drop_percent)) #用random選擇要drop的index
    aug[:, drop] = 0 #把要drop的設為0
